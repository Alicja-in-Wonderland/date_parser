...
pub fn get_date(input_handle: &Stdin) -> date::Date {
    ...
    let weekday = date::Weekday::parse(split_date[0]);
    let month = date::Month::parse(split_date[2]);
...
pub fn get_preferred_language(input_handle: &Stdin) -> crate::Language {
    ...
    if selected_language.as_str() == "en" {
        ...
        language = crate::Language::Polish;
    } else {
